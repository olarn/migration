/*
	04 - Transform and migrate case_transaction_sla_hop (Java logic adapted)
	- Deterministic case_sla_hop_id from case + owner team + start/end
*/
...
BEGIN TRY
	...
	WITH base AS (
		SELECT
			...
			so.End_Date_Time__c,
			LEFT(COALESCE(so.Owner_Team_NEW, so.Owner_Team__c), 255) AS team_name,
			LEFT(so.Name, 255) AS owner_name,
		...
		SELECT *, CONVERT(varchar(32), HASHBYTES('MD5', UPPER(CONCAT(ISNULL(Case__c,''),'|',ISNULL(Owner_Team__c,''),'|',ISNULL(Start_Date_Time__c,''),'|',ISNULL(End_Date_Time__c,'')))), 2) AS md5hex
		FROM base
	), ids_fmt AS (
