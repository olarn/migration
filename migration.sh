...
echo "[2/6] Waiting for SQL to be ready..."
# Wait for the mssql-tools container to be up
for i in {1..60}; do
  if docker compose ps mssql-tools | grep -q "running"; then
    break
  fi
  sleep 2
done
...
for i in {1..60}; do
  if docker compose exec -T mssql-tools /opt/mssql-tools/bin/sqlcmd -S mssql -U sa -P 'Str0ngP@ssw0rd!' -Q "SELECT 1" >/dev/null 2>&1; then
    break
  fi
  sleep 2
done

echo "[3/6] Preparing SQL directories inside container..."
...
echo "[4/6] Copying SQL files into container..."
...
echo "[5/6] Creating databases (if needed)..."
...
echo "[5.1/6] Creating/ensuring staging tables (ignoring if already exist)..."
docker compose exec -T mssql-tools /bin/bash -lc \
...
echo "[5.2/6] Creating/ensuring destination tables (ignoring if already exist)..."
docker compose exec -T mssql-tools /bin/bash -lc \
