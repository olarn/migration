...
@Component
@Slf4j
public class CompletedToTempProcessor implements ItemProcessor<StgCaseInProgressModel, TempStgCaseInProgressLogModel> {

    private final BatchMigrationDataRequest batchRequest;
    @Value("#{stepExecution}")
    private StepExecution stepExecution;

    public CompletedToTempProcessor(@Value("#{jobParameters['batchRequest']}") String batchRequestJson) {
        try {
