...
@Component
@Slf4j
public class ResolvedToTempProcessor implements ItemProcessor<StgCaseInProgressModel, TempStgCaseInProgressLogModel> {

    private final BatchMigrationDataRequest batchRequest;
    @Value("#{stepExecution}")
    private StepExecution stepExecution;

    public ResolvedToTempProcessor(@Value("#{jobParameters['batchRequest']}") String batchRequestJson) {
        try {
    ...
    public TempStgCaseInProgressLogModel process(StgCaseInProgressModel item) {
        try {
            ...
            return new TempStgCaseInProgressLogModel(
                    item,
                    batchRequest,
                    jobExecutionId, BatchRecordStatusEnum.PENDING
            );
        } catch (Exception e) {
